      ...
      {items.map((item) => (
        ...
        >
          <AnimatePresence mode="popLayout">
            {hoveredIndex === item.id && (
              ...
              >
                ...
                <div className="text-background/70 text-xs">{item.designation}</div>
              </motion.div>
            )}
          </AnimatePresence>
          ...
          <img
            ...
            src={item.image}
            alt={item.name}
            className="!m-0 !p-0 object-top rounded-full h-14 w-14 border-2 group-hover:scale-105 group-hover:z-30 border-background relative transition duration-500 object-fill"
          />
        </div>
      ))}
    </div>
